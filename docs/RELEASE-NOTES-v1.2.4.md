...
- **Audience-auth on long-lived**: `ApplicationUserCache` is populated at upgrade by the audience-auth claims-transformer's role-resolver path (`ApplicationUserRoleResolverAdapter`). The source adapter copies it onto `Connection.Items` at upgrade and seeds it onto every per-invocation `Items` bag. `UserStateAccessor` reads `invocation.Items` and HITS. The lazy-resolve path doesn't fire. The new propagation line is dead-code in this scenario.
