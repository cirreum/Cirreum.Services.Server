...
internal sealed partial class WebSocketOrchestrator(
	...
	internal async Task HandleWebSocketAsync(
		...
		// IApplicationUserResolver matches. Per-message WebSocketInvocationContext
		// construction seeds per-invocation Items from these slots, so consumers like
		// UserStateAccessor read uniformly across HTTP and WebSocket without hitting the IdP
